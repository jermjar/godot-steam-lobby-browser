...
func _ready() -> void:
	if SteamInit.steam_enabled == false:
		multiplayer_button.disabled = true
	
	# Initialize to show MainMenuButtons always
...
## Triggers whenever you type in the LobbyNameInput field in the CreateLobbyContainer
...
## Handles joining off of Steam invite/friends list
	...
	if arguments.size() > 0:
		...
		if arguments[0] == "+connect_lobby":
			...
			if int(arguments[1]) > 0:
				# At this point, you'll probably want to change scenes. Something like a loading into lobby screen
